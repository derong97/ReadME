...
class User:
    ...
    def signup(self):
        ...
        # Insert user record into db
            ...
            return {"token": token.decode('UTF-8'), "reviewerID": f"{user['_id']}", "username": user['name'],"message" : f"{user['name']} successfully signed up"}, 200
    ...
    def login(self):
        ...
        if user and pbkdf2_sha256.verify(request.json.get('password'), user['password']):
            # token expires after 30 min
            token = jwt.encode({"reviewerID": f"{user['_id']}", 'exp' : datetime.datetime.utcnow() + datetime.timedelta(minutes=30)}, SECRET_KEY)
            return {"token": token.decode('UTF-8'), "reviewerID": f"{user['_id']}", "username": user['name'], "message" : f"{user['name']} successfully logged in"}, 200
